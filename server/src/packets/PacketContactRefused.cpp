...
PacketContactRefused::  PacketContactRefused()
        : APacket(APacket::ID_CONTACT_REFUSED, APacket::SERVER_TO_CLIENT)
{
...
void                PacketContactRefused::unserialize(SerializerBuffer *inBuffer)
{
    uint16_t        size;
...
SerializerBuffer    *PacketContactRefused::serialize() const
{
...
void                PacketContactRefused::setUserData(uint32_t uid,
...
uint32_t            PacketContactRefused::getUserId() const
{
        return (this->_contact.uid);
}

std::string const   &PacketContactRefused:: getUsername() const
{
        return (this->_contact.username);
}

std::string const   &PacketContactRefused:: getStatus() const
{
